                ...
                <el-table-column :label="tableHead1">
                    ...
                    label="订单号">
                        <template slot-scope="scope1">
                            ...
                            <el-button @click="openDialog(scope1.row.ORDER_NO,scope1.row.STATUS_ID)" type="text">{{scope1.row.ORDER_NO}}</el-button>
                        </template>
                    </el-table-column>
                        <el-table-column
                        label="订单状态">
    ...
    searchByMonth(){
        ...
        searchAssignments(url,data).then(res=>{
            ...
            for(let i=0;i<zoom.length;i++){
                zoom[i].sumMoney=zoom[i].ALL_SPEND+zoom[i].ALLBACK_Y+zoom[i].ALLBACK_M;
