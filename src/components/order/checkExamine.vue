...
<el-card class="centerCard">
 <el-dialog
  ...
  <!-- <span slot="footer" class="dialog-footer">
    <el-button @click="saveChange()" style="float:left;" type="success">保存修改</el-button>
  ...
  <div slot="header">
    ...
    <el-card class="tableCard" shadow="hover">
      <div slot="header">
        ...
        <span class="zoomLeft">联系方式：<span class="zoomRight">{{ruleForm.TELEPHONE}}</span></span>           
      </div>
        <el-table
          border
          ...
          :row-class-name="tableRowClassName">
              ...
              <el-table-column  v-if="check_CURTAIN_STATUS_ID==1"
              align="center" 
              prop="checkStatus"
              label="是否修改">
              </el-table-column>
        ...
        <div style="float:right;margin-top:20px;height:100px;">
          ...
          <el-button v-if="check_CURTAIN_STATUS_ID==2" @click="_defeat()" size="medium" type="warning">退回兰居修改</el-button>
          <el-button v-if="check_CURTAIN_STATUS_ID==2" @click="_pass()" size="medium" type="success">确认审核通过</el-button>
          <el-button :disabled="exButton" v-if="check_CURTAIN_STATUS_ID==1" @click="LjExamine()" size="medium" type="success">修改并再次审核</el-button>
        </div>   
...
import { getOrderlist,passExamine,orderDetail,defeatChange } from '@/api/orderList'
...
import DetailCurtainTable from '../detail/detailCurtainTable'
  export default {
    ...
    getFinalData(msg){
      ...
      /* if(this.allCurtains.length==0){
          this.allCurtains.push(msg);
          console.log(this.allCurtains);
          return
        }
        console.log(this.allCurtains.length)
          for(let i=0;i<this.allCurtains.length;i++){
            if(this.allCurtains[i][0].lineNo==msg[0].lineNo){
              this.allCurtains[i]=msg;
              console.log('替换');
              console.log(this.allCurtains[i][0].lineNo,i);
            }
            else{
              this.allCurtains.push(msg);
            }
    ...
    openDialog(tab,index){
        ...
        this.headerData.location=(tab.CURTAIN_ROOM_NAME)?tab.CURTAIN_ROOM_NAME:'无';//'无';
