...
<!-- 地址管理信息 -->
      <el-dialog width="70%"  @close="dialogClose" title="管理收货信息" :visible.sync="dialogFormVisible">
        <el-button @click="clickNew()" type="success">添加地址</el-button>      
              <el-table
                ...
                label="操作">
                <template slot-scope="scope">
                  <el-button v-show="scope.row.addressId!=0" @click="editIt(scope.row)" type="warning" size="small">编辑</el-button>
                  <el-button v-show="scope.row.addressId!=0" @click="deleteIt(scope.row)" type="danger" size="small">删除</el-button>
              ...
              <div>
                <el-form :model="form" ref="form" class="demo-ruleForm" :rules="formRules">
                  ...
                  <el-form-item label="联系方式" :label-width="formLabelWidth" prop="telephone">
                    <el-input style="width:90%;" v-model="form.telephone" autocomplete="off" placeholder="请输入联系电话"></el-input>
                  </el-form-item>
                </el-form>
    ...
    <el-card class="childCard"  shadow="hover">
        <div slot="header">
          <span class="zoomLeft">甲方：</span><span class="zoomRight">广东玉兰集团股份有限公司</span>
          <span class="zoomLeft">乙方：</span><span class="zoomRight">{{realName}}</span>
          <span class="zoomLeft">经办人：</span><span class="zoomRight">{{chargeData.CUSTOMER_AGENT}}({{chargeData.OFFICE_TEL}})</span>
        </div>
        <div :class="overflow">  <!-- style="height:160px; overflow:auto;"用三元运算改变高度 -->
          <p  v-for="(item,index) of data" :key="index" >
            <el-radio @change="showAddress"  v-model="radio" :label="index" border>
            {{item.wlContacts}} ({{item.wlTel}}) {{item.province}}{{item.city}}{{item.country}}{{item.postAddress}}
            </el-radio>
      ...
      <div class="grayDiv">  <!-- 配送信息 -->
            <span>选择配送方式：</span>
              <el-select @change="changePeiSong" style="width:30%; display:inline-block;margin-right:30px;" v-model="ctm_order.deliveryType" placeholder="请选择">
                <el-option
            ...
            <span>物流公司：</span>
            <el-input  style="width:30%;" :disabled="this.ctm_order.deliveryType==3? false:true" v-model="ctm_order.deliveryNotes" placeholder="物流备注"></el-input><br><br>
            <span>购买人：</span>
            ...
            <el-input style="width:30%;" v-model="ctm_order.buyUserPhone" placeholder="请输入联系方式"></el-input><br>
            <span>备注：</span>
            <el-input type="textarea" maxlength="200" :autosize="{ minRows:3, maxRow:4}" resize="none" v-model="ctm_order.notes" placeholder="请输入订单备注(200字以内，任何发货信息写在备注无效！)"></el-input>
            <span style="color:#ccc">{{ctm_order.notes.length}}/200</span><br>   
            <span>工程报备单号：</span>
          ...
          :row-class-name="tableRowClassName">
              ...
              label="应付金额">
                 <template slot-scope="scope1">
                    ...
                    <span v-else>{{scope1.row.finalPrice-scope1.row.yuefanli-scope1.row.nianfanli | priceFilter}}</span>
                 </template>
              </el-table-column>      
        </el-table>
    </el-card>  <!-- 使用优惠券 -->
      ...
      <el-collapse v-model="activeNames">
            <el-collapse-item title="使用优惠券/礼品卡" name="1">
              ...
              <div v-if="item.dateId!='0'" v-for="(item,index) of couponData" :key="index" class="cctv"><!-- @click="useCoupon(item.id,item.rebateType)"-->
                ...
                <div class="couponBody">
                  ...
                  <div style="margin:0 auto; width:245px;">
                    ...
                    <el-checkbox v-model="couponStatus[index]" @change="changeCoupon(couponStatus[index],item.id,item.rebateType)"></el-checkbox>
                  </div>
       ...
       <div class="rightDiv">
           <p v-if="isManager === '0'">折后总金额：<span>***</span></p>
           <p v-else>折后总金额：<span>{{totalPrice| priceFilter}}</span></p>
           <p v-if="isManager === '0'">总返利：<span>***</span></p>
           <p v-else>总返利：<span>{{backPrice| priceFilter}}</span></p>
           <p v-if="isManager === '0'">应付总金额：<span>***</span></p>
           <p v-else>应付总金额：<span>{{allSpend| priceFilter}}</span></p>
           ...
           <el-button v-if="!curtainOrOther" @click="payCurtain()" size="medium" type="danger" plain>确认提交</el-button>
       </div>
   </el-card> 
  ...
  export default {
    ...
    data() {
      return {
        ...
        couponData:[{/* 
            application: "可购买全部产品", 
            customerCode: "C01613",
            dateEnd: 1577721600000,
            dateId: null,
            dateStart: 1546272000000,
            id: "Y2019-C01613",
            notes: "玉兰E券",
            rebateMoney: 10000,
            rebateMoneyOver: 10000,
            rebateType: "year",
            status: "1", */
        }],
        rebateY:'',
    ...
    methods: {
      ...
      changeCoupon(index,id,type){
        ...
        /* if(type=='year'){
          if(index==true){
            console.log('这是一张选中的年券'); 
            //this.rebateY=id;
            console.log(this.rebateY);
            //console.log(this.rebateY);
            usetheCoupon(url,data).then(res => {
              console.log(res);
              for(var i=0;i<res.data.rebate.length;i++){
            ...
            console.log('这是一张取消选中的年券');
            this.rebateY='';   //清空数据防止提交时错误
          ...
          if(index==true){
            ...
            console.log('这是一张取消选中的月券');
            this.rebateM='';  //清空数据防止提交时错误
            console.log(this.rebateM);
             for(var i=0;i<this.array2.length;i++){
      ...
      //使用优惠券
        ...
        usetheCoupon(url,data).then(res => {
            console.log(res);
            this.totalPrice=res.data.allspend;
        
            for(var i=0;i<res.data.rebate.length;i++){
              this.array2[i].yuefanli=res.data.rebate[i].rebateMonth;
            }
            for(var k=0;k<this.array2.length;k++){
              this.array2[k].promotionCost=(this.array2[k].promotionCost-this.array2[k].nianfanli-this.array2[k].yuefanli).toFixed(2);
            }
            this.ORDERBODY=this.array2;
      ...
      changeAddress(formName){
        ...
        this.$refs[formName].validate((valid) => {
          if (valid &&data.province!=undefined && data.city!=undefined &&this.form.address!=undefined&&this.form.name!=undefined&&data.province!='' && data.city!='' &&this.form.address!=''&&this.form.name!=''&&this.form.telephone!=''&&this.form.telephone!=undefined) {
      ...
      printfCountry(){
        ...
        //console.log(this.country[country].regionName);
        //console.log(this.country[country].regionId);
      ...
      getCountry(){
        ...
        //console.log(this.city[city].regionName);
        //console.log(this.city[city].regionId);
        ...
        this.form.shiID=this.city[city].regionId;
            Axios.post('/areaRegion/getCountry.do', {
              ...
              "parentCity":this.city[city].parentCity,
              "parentArea":this.city[city].parentArea,
              "parentProvince":this.city[city].parentProvince,
              "regionLevel": this.city[city].regionLevel,
              "country": this.city[city].country,
              "dateCre": this.city[city].dateCre,
              "dateUpdate": this.city[city].dateUpdate,
              "cityType": this.city[city].cityType,
              "parentDistrict": this.city[city].parentDistrict */
             })
            .then((res) =>{
              console.log(res);
              this.country=res.data.country;
      ...
      getCity(){
        ...
        //console.log(this.province[shengfen].regionName);
            Axios.post('/areaRegion/getCity.do', {
              "regionId":this.province[shengfen].regionId,
              "regionName":this.province[shengfen].regionName,/* 
              "parentCity":this.province[shengfen].parentCity,
              "parentArea":this.province[shengfen].parentArea,
              "parentProvince":this.province[shengfen].parentProvince,
              "regionLevel": this.province[shengfen].regionLevel,
              "country": this.province[shengfen].country,
              "dateCre": this.province[shengfen].dateCre,
              "dateUpdate": this.province[shengfen].dateUpdate,
              "cityType": this.province[shengfen].cityType,
              "parentDistrict": this.province[shengfen].parentDistrict */
              })
            .then((res) =>{
              console.log(res);
              this.city=res.data.city;
      ...
      showAddress(){
          ...
          if(this.addressIt==false)
          {
            ...
            this.radio=0;
              ...
              //console.log(abcd);   
              /* console.log(this.ctm_order.reciverArea1); 
              console.log(this.ctm_order.reciverArea2); 
              console.log(this.ctm_order.reciverArea3); 
              console.log(this.ctm_order.postAddress); 
              console.log(this.ctm_order.wlTel); 
              console.log(this.ctm_order.wlContacts); */     
          }
        },
      ...
      huodongjia(){
        ...
        for(var i=0;i<getPush.length;i++){
          ...
          if(getPush[i].quantity==null){
            ...
            this.array[i].prime_cost=this.subtotal(getPush[i].width,getPush[i].height,getPush[i].price)
          }
          else{
      ...
      orderList(){
        ...
        for(var i=0;i<getPush2.length;i++){
          ...
          if(getPush2[i].unit=='平方米'){
            this.array2[i].qtyRequired=Math.round(getPush2[i].height*getPush2[i].width*100)/100;
          }
          else{
            this.array2[i].qtyRequired=getPush2[i].quantity;
          }
      ...
      payCurtain(){
        ...
        var data ={
          ...
          if(this.ctm_order.deliveryNotes==''&& this.ctm_order.deliveryType==3){
          ...
          curtainPay(url,data).then(res =>{
            console.log(res);
            console.log('成功!!!');
            if(/* this.totalPrice */this.allSpend>this.Initial_balance && this.arrearsFlag!='N'){
      ...
      payIt(){
        ...
        var data ={
          ...
          submitOrder(url,data).then(res => {
            ...
            /* DeleteShopRecord(deleteArray).then(res => {
              console.log(res);
              console.log('删除成功');
            }) */
